...
public class DiscordMapper {
  ...
  public static DiscordWebhookRequest mapToDiscordWithServerAlarm(
    ...
    return new DiscordWebhookRequest(
        List.of(
            new Embed(
                title,
                description,
                DiscordColor.RED.getCode(),
                fields,
                new Footer(LocalDateTime.now().toString()))));
  }

  public static DiscordWebhookRequest mapToDiscordWithInformation(
      String title, List<Field> fields) {
    return new DiscordWebhookRequest(
        List.of(
            new Embed(title, null, DiscordColor.LIGHT_BLUE.getCode(), fields, new Footer(null))));
  }

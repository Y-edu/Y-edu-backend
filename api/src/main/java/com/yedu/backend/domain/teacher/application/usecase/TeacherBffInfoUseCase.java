...
public class TeacherBffInfoUseCase {
    ...
    public TeacherAllInformationResponse retrieveAllInformation(String token) {
        ...
        if (classType.equals(ClassType.수학)) {
            subjectInfoFuture = CompletableFuture.supplyAsync(() -> teacherInfoUseCase.teacherMathDetails(teacherId));
            curriculumInfoFuture = CompletableFuture.supplyAsync(() -> teacherInfoUseCase.curriculumMath(teacherId));
        } else if (classType.equals(ClassType.영어)) {
            subjectInfoFuture = CompletableFuture.supplyAsync(() -> teacherInfoUseCase.teacherEnglishDetails(teacherId));
            curriculumInfoFuture = CompletableFuture.supplyAsync(() -> teacherInfoUseCase.curriculumEnglish(teacherId));
        }

        CompletableFuture.allOf(commonsFuture, districtAndTimeFuture, subjectInfoFuture, curriculumInfoFuture).join();
        ...
        if (classType.equals(ClassType.수학)) {
            return buildMathTeacherInfo(builder, (TeacherMathResponse) subjectInfo, (MathCurriculumResponse) curriculumInfo);
        }

        if (classType.equals(ClassType.영어)) {
            return buildEnglishTeacherInfo(builder, (TeacherEnglishResponse) subjectInfo, (EnglishCurriculumResponse) curriculumInfo);
        }

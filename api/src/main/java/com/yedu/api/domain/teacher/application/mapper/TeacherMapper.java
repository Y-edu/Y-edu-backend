package com.yedu.api.domain.teacher.application.mapper;
...
import com.yedu.api.domain.teacher.domain.entity.constant.University;
...
public class TeacherMapper {
  ...
  private static TeacherClassInfo mapToTeacherClassInfo(TeacherInfoFormRequest request) {
    // style enum으로 사용 예정
    TeacherClassInfo.TeacherClassInfoBuilder builder =
        TeacherClassInfo.builder()
  ...
  public static TeacherDistrict mapToTeacherDistrict(Teacher teacher, String region) {
    return TeacherDistrict.builder().district(District.fromString(region)).teacher(teacher).build();
  }
  ...
  public static TeacherEnglish mapToTeacherEnglish(
      Teacher teacher, TeacherInfoFormRequest request) {
  ...
  public static TeacherCommonsInfoResponse mapToTeacherCommonsInfo(Teacher teacher) {
    ...
    return new TeacherCommonsInfoResponse(info.getProfile(), info.getNickName());
  }
  ...
  public static TeacherEnglishResponse mapToTeacherEnglish(
      Teacher teacher, TeacherEnglish english) {
    TeacherClassInfo classInfo = teacher.getTeacherClassInfo();
    TeacherSchoolInfo schoolInfo = teacher.getTeacherSchoolInfo();
  ...
  private static List<String> getForeignExperiences(TeacherEnglish english) {
    ...
    if (english.getForeignExperience() != null) {
      ...
      foreignExperiences =
          Arrays.stream(foreignExperience.split("\n")).toList().stream().map(String::trim).toList();
    }
  ...
  public static EnglishCurriculumResponse mapToEnglishCurriculumResponse(
      TeacherInfo teacherInfo, TeacherEnglish english) {
    return new EnglishCurriculumResponse(english.getTeachingStyle(), teacherInfo.getVideo());
  }

  public static DistrictAndTimeResponse mapToDistrictAndTimeResponse(
      List<String> districts, Map<Day, List<LocalTime>> availableTimes) {
  ...
  public static TeacherInfoResponse mapToTeacherInfoResponse(
      Teacher teacher, List<String> districts, Map<Day, List<LocalTime>> available) {
    boolean alarmTalk = teacher.getStatus() == TeacherStatus.활동중;
    return new TeacherInfoResponse(
        teacher.getTeacherInfo().getName(), alarmTalk, districts, available);
  }
}

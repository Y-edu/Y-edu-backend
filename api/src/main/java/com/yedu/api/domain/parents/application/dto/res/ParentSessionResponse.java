...
public record ParentSessionResponse(
  ...
  public static List<ParentSessionResponse> of(Map<ClassSession, List<SessionChangeForm>> sessionsMap) {
    if (sessionsMap.isEmpty()) {
      return Collections.emptyList();
    }
    List<ClassSession> rawSessions = new ArrayList<>(sessionsMap.keySet());

    Map<ClassMatching, List<ClassSession>> groupedByMatching = rawSessions.stream()
        .collect(Collectors.groupingBy(cs -> cs.getClassManagement().getClassMatching()));

    Map<Long, List<SessionChangeForm>> changeFormMap = sessionsMap.entrySet().stream()
    ...
    for (Map.Entry<ClassMatching, List<ClassSession>> entry : groupedByMatching.entrySet()) {
      ...
      List<ClassSession> calculatedSessions = new ClassSessions(sessions).calculateRoundWithReset(maxRoundNumber);

      List<Session> sessionResponses = calculatedSessions.stream()
          .map(it -> {
            List<SessionChangeForm> forms = changeFormMap.get(it.getClassSessionId());
            Map<SessionChangeType, Boolean> submits = Arrays.stream(SessionChangeType.values())
                .collect(Collectors.toMap(
                    type -> type,
                    type -> forms != null && forms.stream().anyMatch(form -> form.getChangeType() == type)
                ));
      ...
      ParentSessionResponse response = ParentSessionResponse.builder()

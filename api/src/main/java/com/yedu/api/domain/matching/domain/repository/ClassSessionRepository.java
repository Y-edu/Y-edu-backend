...
public interface ClassSessionRepository
  ...
  Optional<ClassSession>
      findFirstByClassManagementAndSessionDateBeforeAndCompletedTrueAndCancelFalseAndTeacherRoundIsNotNullOrderBySessionDateDesc(
  ...
  """)
  @Modifying
  void updateRoundBySessionId(
      @Param("sessionId") Long sessionId, @Param("teacherRound") Integer teacherRound);
  ...
  """)
  @Modifying
  void updateSessionDateBySessionId(
      @Param("sessionId") Long sessionId, @Param("newDate") LocalDate newDate);
...
""")
  ...
  List<ClassSession> findByClassManagement_ClassManagementId(Long classManagementId);

  @Query(
      """
    select count(cs) from ClassSession cs
    where cs.classManagement.classManagementId = :classManagementId
      and cs.payStatus = :payStatus
      and ((cs.completed = true) or (cs.isTodayCancel = true and cs.cancelReason = :cancelReason))
  """)

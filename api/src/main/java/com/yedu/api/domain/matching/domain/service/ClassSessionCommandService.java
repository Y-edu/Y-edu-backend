...
public class ClassSessionCommandService {
  ...
  public Integer getSessionCountPendingPayStatusAndCompleted(Long classManagementId) {
    return classSessionRepository.findCountByClassManagementIdPendingPayStatusAndCompleted(
        classManagementId, PayStatus.PENDING, CancelReason.PARENT.name());
  }
                                ...
                                .or(
                                    () ->
                                        Optional.ofNullable(classManagement.getFirstDay())
                                            .map(
                                                firstDay ->
                                                    firstDay.isBefore(today) ? today : firstDay))
                                .orElse(today),
                            date ->
                                !date.isAfter(
                                    today.plusMonths(2).with(TemporalAdjusters.lastDayOfMonth())),
  ...
  public ClassSession revertCancel(Long sessionId) {
    ...
    // 3. classManagementId & sessionDate(DATE_FORMAT YYYYMM)와 일치하는 sesionDate, 무료 보강 제외한 조건인
    // classSessions 목록 조회
    ...
    List<ClassSession> allClassSessions =
        classSessionRepository.findByClassManagementIdAndYearMonth(
  ...
  public ClassSession complete(Long sessionId, CompleteSessionRequest request) {
    ...
    // FIXME :: 결제 선생 연동 후 입금날짜 기준으로 회차 다시 기록해야함, 현재로직은 이전과외일정 기준으로 회차를 설정하는데, 이전 과외 일정이 없거나 신규 과외는
    // 회차가 기록되지 않는 이슈가 있음
  ...
  public Pair<ClassSession, LocalDate> change(
      Long sessionId, LocalDate sessionDate, LocalTime start) {
    ClassSession session = findSessionById(sessionId);
  ...
  public void createSingleSessions(
    ...
    log.info(
        ">>> 과외 일정 생성 : classManagementId = {}, forceCreate = {}, changeStartDate = {}",
  ...
  public void updateRoundSequentially(Long sessionId) {
    List<ClassSession> currentSessions =
        classSessionRepository.findBySameClassManagementId(sessionId);
    ...
    if (currentSession.getCancelReason() == CancelReason.TEACHER.name()) {
      ...
      if (currentIndex != -1 && currentIndex + 1 < currentSessions.size()) {
        ...
        System.out.println(
            "No next session found. currentIndex: "
                + currentIndex
                + ", size: "
    ...
    List<ClassSession> allSessionsInMonth =
    ...
    for (int i = 0; i < allSessionsInMonth.size(); i++) {
      ...
      if (session.isTodayCancel() && "TEACHER".equals(session.getCancelReason())) {
        ...
        if (i + 1 < allSessionsInMonth.size()) {
          ...
          System.out.println(
              "TEACHER 취소 다음 세션 teacherRound를 0으로 설정: " + nextSession.getClassSessionId());
        }
      }
    }
    List<ClassSession> allClassSessions =
    ...
    if (!allClassSessions.isEmpty()) {
      ...
      for (ClassSession session : allClassSessions) {
        ...
        classSessionRepository.updateRoundBySessionId(
            session.getClassSessionId(), currentTeacherRound);
        System.out.println(
            "정상 세션 teacherRound 설정: " + session.getClassSessionId() + " -> " + currentTeacherRound);
        currentTeacherRound++;
      }
    }
  }
  ...
  public void updateDateAndReorderRounds(Long sessionId) {
    ...
    LocalDate currentSessionDateEndOfMonth =
        currentSessionDate.withDayOfMonth(currentSessionDate.lengthOfMonth());

    // 현재 월의 세션들 업데이트
    List<ClassSession> currentMonthSessions =
        classSessionRepository.findByClassManagementAndSessionDateBetween(
            currentSession.getClassManagement(),
            currentSessionDateStartOfMonth,
            currentSessionDateEndOfMonth);
    List<ClassSession> currentValidSessions =
    ...
    if (currentSessionDate.getMonth() != requestedDate.getMonth()
      ...
      LocalDate requestedSessionDateEndOfMonth =
          requestedDate.withDayOfMonth(requestedDate.lengthOfMonth());

      List<ClassSession> requestedMonthSessions =
          classSessionRepository.findByClassManagementAndSessionDateBetween(
              currentSession.getClassManagement(),
              requestedSessionDateStartOfMonth,
              requestedSessionDateEndOfMonth);
      List<ClassSession> requestedValidSessions =
          requestedMonthSessions.stream()
              .filter(
                  session -> session.getTeacherRound() != null && session.getTeacherRound() != 0)
              .sorted(Comparator.comparing(ClassSession::getSessionDate))
              .toList();
      ...
      if (!requestedValidSessions.isEmpty()) {
        ...
        for (ClassSession session : requestedValidSessions) {
          classSessionRepository.updateRoundBySessionId(
              session.getClassSessionId(), requestedTeacherRound);
          System.out.println(
              "변경할 월 정상 세션 teacherRound 설정: "
                  + session.getClassSessionId()
                  + " -> "
  ...
  public void updateRoundForGeneralCancel(Long sessionId) {
    ...
    List<ClassSession> validSessions =
        allSessionsInMonth.stream()
            .filter(session -> session.getTeacherRound() != null && session.getTeacherRound() != 0)
            .sorted(Comparator.comparing(ClassSession::getSessionDate))
            .toList();

...
public class DiscordEventMapper {
  public static ScheduleCancelEvent mapToScheduleCancelEvent(
      Teacher teacher, ApplicationForm applicationForm, String refuseReason) {
  ...
  private static String safeText(Supplier<String> supplier) {
    return Optional.ofNullable(supplier.get()).filter(StringUtils::hasText).orElse("(이름 미입력)");
  }

  public static TeacherRegisterEvent mapToTeacherRegisterEvent(
      Teacher teacher, List<TeacherDistrict> districts) {
    ...
    return new TeacherRegisterEvent(
        teacherInfo.getName(),
        teacherInfo.getNickName(),
        subject.toString(),
        teacherLink.toString(),
        regions.toString());
  }
}

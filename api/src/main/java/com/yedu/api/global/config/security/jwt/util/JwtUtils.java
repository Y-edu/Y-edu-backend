package com.yedu.api.global.config.security.jwt.util;

import static com.yedu.api.global.config.security.jwt.constant.Role.*;
import static com.yedu.api.global.config.security.jwt.constant.Type.*;
import static org.springframework.http.HttpStatus.BAD_REQUEST;

import com.yedu.api.admin.domain.entity.Admin;
import com.yedu.api.global.config.security.jwt.auth.AuthDetails;
import com.yedu.api.global.config.security.jwt.auth.AuthDetailsService;
import com.yedu.api.global.config.security.jwt.constant.Role;
import com.yedu.api.global.config.security.jwt.constant.Type;
...
public class JwtUtils {
  ...
  public String generateRefreshToken(Long id, Role role) {
    ...
    redisRepository.setValues(
        REFRESH.toString() + id + role, refreshToken, Duration.ofSeconds(refreshExpiration));
    return refreshToken;
  }
  ...
  public String checkRedis(Long id, HttpServletRequest request) {
    ...
    String redisToken =
        redisRepository.getValues(REFRESH.toString() + id + claims.get(ROLE)).orElseThrow();
    if (!redisToken.equals(refreshToken)) throw new IllegalArgumentException();
  ...
  private void jwtExceptionHandler(
      HttpStatus status, HttpServletResponse response, ApplicationException ex) {
